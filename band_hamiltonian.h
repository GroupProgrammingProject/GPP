...
double band_Hamiltonian(int n,Eigen::MatrixXd* modr, Eigen::MatrixXd* rx,Eigen::MatrixXd* ry,Eigen::MatrixXd* rz,Eigen::MatrixXd* eigvects, bool verbose){
  ...
  Eigen::MatrixXcd Hijab(4*n,4*n);			   							  // Hamiltonian matrix for diagonalization
      ...
      d.at(0)=rxloc; d.at(1)=ryloc; d.at(2)=rzloc;
      	...
      	for (a=0;a<4;a++) {                               // Cycle through orbitals of atom i
				for (b=0;b<4;b++) {                            // Cycle through orbitals of atom i
	  				...
	  				Hijab(4*i+a,4*j+b)     =std::complex<double>(sr*hijab,1);          // Scale hijab and populate matrix Hijab
