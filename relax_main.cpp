...
int main(int argc, char* argv[]){
  /*if (argc<1){std::cout<<"You should append a file to the main object!"<<std::endl;}
	...
	double dt=1,T=500,Tf,m=12*1.0365e2,rc=2.6,rv=3,tmd,kb=1./11603,fmax,ftot=1e3,ftotold,h=0.002,gam=1;
	...
	do{
	   if(count*100%nmax==0){std::cout << count*100/nmax << "% complete\tftot = " << ftot << std::endl;}
		ftotold = ftot;
		ftot = 0;
		avekforces(n,norbs,rc,&rx,&ry,&rz,&modr,&nnear,&inear,&fx,&fy,&fz,&kpoints);
		fprintf(file,"%d\nC12 \t %.3f \t %.3f \t %.3f \n",n,lats[0],lats[1],lats[2]);
		for(i=0; i<n; i++){
		...
		for(i=0; i<n; i++){ftot = ftot + fmag.at(i);} // find the sum of all magnitudes of forces
		fmax=*std::max_element(fmag.begin(),fmag.end()); //find largest elemen of fmag
		GetDistances(&modr,&rx,&ry,&rz,&posx,&posy,&posz,&lats,rv,pbc);
		NearestNeighbours(&inear,&nnear,&modr,rv);
		//ebs=Hamiltonian(n,&modr,&rx,&ry,&rz,&eigvects,v);
		count=count+1;
	}while(fmax>1e-5 && count<nmax && (ftot <= ftotold || ftot > 1e-3)); //continue until desired accuracy reached, or we've reached nmax steps
