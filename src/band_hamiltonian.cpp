		...
		if (i == j) {sr = 1; kr = 0;}                        // Don't apply scaling function if i=j
		  ...
		  kr = dot(k,&d)*r;
				...
				for (b=0;b<4;b++) {                            // Cycle through orbitals of atom i
	  				...
	  				Hijab(4*i+a,4*j+b) = exp(std::complex<double>(0,1)*kr)*sr*hijab;       // Scale hijab and populate matrix Hijab
	  				Hijab(4*j+b,4*i+a) = exp(-std::complex<double>(0,1)*kr)*sr*hijab;      // Scale hijab and populate matrix Hijab
				}                                              // End loop over b
      	}                                                 // End loop over a
    	}                                                    // End loop over j
  	}                                                       // End loop over i
