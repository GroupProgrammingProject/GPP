#include "../include/complex_solver_ham.h"
...
double Hamiltonian(int n, std::vector<int>* type, std::vector<double>* posx, std::vector<double>* posy, std::vector<double>* posz, std::vector<double>* eigvects){
  ...
  for(i=0;i<4*n;i++){
  		eigvalarr.push_back(pair(real(eigvals(i,0)),i));
//  		std::cout << "eigvalarr no. " << eigvalarr.at(i).second << " is " << eigvalarr.at(i).first << std::endl;
  ...
  for(i=0;i<4*n;i++){
  		ind=eigvalarr.at(i).second;
		std::cout << "index of current eigval is " << ind << std::endl;
	  for(j=0;j<4*n;j++){
			(*eigvects).at(i*4*n+j)=real(eigvectors(j,ind));
...
/*  std::cout << "Sorted eigenvectors:" << std::endl;
  for(i=0;i<4*n;i++){
		std::cout << "for eigenvalue(" << eigvalarr.at(i).second << ") = " << eigvalarr.at(i).first << std::endl;
		for(j=0;j<4*n;j++){
			std::cout << (*eigvects).at(i*4*n+j) << std::endl;
		}
	}
*/
