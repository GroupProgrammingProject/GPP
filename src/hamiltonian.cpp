...
double Hamiltonian(int n,int norbs,std::vector<double>* TBparam, Eigen::MatrixXd* modr, Eigen::MatrixXd* rx,Eigen::MatrixXd* ry,Eigen::MatrixXd* rz,Eigen::MatrixXd* eigvects, bool verbose){
  ...
  Eigen::MatrixXd Hijab(n*norbs,n*norbs);			   							  // Hamiltonian matrix for diagonalization
  ...
  for (i=0;i<n;i++) {                                      // Cycle through atoms i
    for (j=0;j<=i;j++) {                                   // Cycle through atoms j (note upper triangular matrix)
		...
		for (a=0;a<norbs;a++) {                               // Cycle through orbitals of atom i
		  for (b=0;b<norbs;b++) {                            // Cycle through orbitals of atom i
			 ...
			 Hijab(norbs*i+a,norbs*j+b)     = sr*hijab;          // Scale hijab and populate matrix Hijab
